...
export function UserDetailDrawer({ user: initialUser, onClose, onUserUpdated }: UserDetailDrawerProps) {
 ...
 const { data: userDataDetail, isFetching, refetch } = useQuery({
 ...
 const user = userDataDetail ?? initialUser;
 if (!user) return null;
 ...
 await Promise.all([
 refetch(),
 onUserUpdated ? Promise.resolve(onUserUpdated()) : Promise.resolve(),
 ]);
 } catch {
 ...
 <>
 <button
 className="fixed inset-0 z-40 bg-slate-950/45 backdrop-blur-sm border-0 cursor-default"
 onClick={onClose}
 tabIndex={-1}
 aria-label="Cerrar"
 type="button"
 ...
 <aside className="fixed right-0 top-0 z-50 flex h-screen w-full max-w-[520px] flex-col overflow-hidden border-l border-border bg-card shadow-2xl animate-in slide-in-from-right duration-300">
 <UserDrawerHeader user={user} model={model} onClose={onClose} isFetching={isFetching} />

 <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
 {isFetching && !userDataDetail ? <DrawerSkeleton /> : null}
...
function UserDrawerHeader({
 ...
 <div>
 <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-primary">Gestion de accesos</p>
 <h2 className="text-lg font-bold text-foreground">Detalle de usuario</h2>
 </div>
 <div className="flex items-center gap-2">
 {isFetching ? <Loader2 className="size-4 animate-spin text-primary" /> : null}
 <button
 type="button"
 onClick={onClose}
 className="rounded-2xl p-2 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30"
 ...
 <h3 className="truncate text-xl font-bold text-foreground">{user.fullName}</h3>
 <p className="mt-1 flex items-center gap-1.5 truncate text-sm text-foreground-soft">
...
function SummaryChip({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
 ...
 <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-foreground-soft">
 <span className="flex size-7 items-center justify-center rounded-xl bg-primary/10 text-primary">{icon}</span>
 {label}
 </div>
 <p className="truncate text-sm font-bold text-foreground">{value || "No informado"}</p>
 </div>
 );
}
...
function UserPermissionsSection({ user, model }: { user: UserProfile; model: UserViewModel }) {
 ...
 <span className="text-xs font-medium text-foreground-soft">Permisos detallados no disponibles</span>
 </div>
 ))}
 </div>
 )}
...
function UserActivityTimeline({ user }: { user: UserProfile }) {
 ...
 <p className="text-xs text-foreground-soft">{activity.date}</p>
 ...
 <p className="text-sm text-foreground-soft">No hay actividad reciente registrada.</p>
 )}
 </DrawerSection>
 );
}
...
function UserDrawerFooter({
 ...
 {!model.hasOperationalAssignment ? (
 <button type="button" className="mt-3 text-xs font-semibold text-primary hover:text-primary-strong" onClick={onLinkWorker}>
 Vincular ficha laboral
 </button>
 ) : null}
 </footer>
...
function DrawerSection({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
 ...
 <span className="flex size-8 items-center justify-center rounded-xl bg-primary/10 text-primary">{icon}</span>
 {title}
 </h4>
 {children}
 </section>
...
function InfoGrid({ items }: { items: Array<[string, string]> }) {
 ...
 <dd className="min-w-0 break-words font-semibold text-foreground">{value || "No informado"}</dd>
 </div>
 ))}
 </dl>
 );
