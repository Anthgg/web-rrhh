...
function UbigeoCell({ loc }: { loc: OrganizationWorkLocation }) {
 ...
 {parts.length > 1 && (
 <span className="text-xs text-muted-foreground">{parts.slice(1).join(", ")}</span>
 )}
 </div>
 );
}
...
export function WorkLocationsWorkspace({ hideHeader }: { hideHeader?: boolean }) {
 ...
 if (isLocationsLoading) return <LoadingPanel title="Cargando lugares de trabajo..." />;

 if (isLocationsError || !locations) {
 return (
 <ErrorState
 ...
 ? locations
 : (locations as any)?.items ?? (locations as any)?.data ?? [];
