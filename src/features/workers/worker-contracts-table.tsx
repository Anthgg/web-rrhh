...
export function WorkerContractsTable({ workerId }: WorkerContractsTableProps) {
 ...
 <div>
 <h3 className="text-sm font-semibold text-foreground">Historial contractual</h3>
 ...
 type="button"
 variant="secondary"
 disabled={isContractsRefetching}
 onClick={() => void refetchContracts()}
 className="h-9 gap-1.5 px-3 text-xs"
 >
 <RefreshCw className={`size-3.5 ${isContractsRefetching ? "animate-spin" : ""}`} />
 Actualizar
 </Button>
 </div>
 ...
 <tr>
 <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-foreground-soft">
 Tipo
 </th>
 <th className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-foreground-soft">
 Fecha
 ...
 <th className="px-4 py-3 text-right text-xs font-semibold uppercase tracking-[0.12em] text-foreground-soft">
 Acciones
 </th>
 </tr>
 </thead>
 ...
 <tr>
 <td colSpan={4} className="px-4 py-10 text-center text-sm text-foreground-soft">
 <div className="flex flex-col items-center justify-center gap-2">
 <span>Este trabajador no tiene contratos registrados.</span>
 <Link href={`/trabajadores/alta?mode=create&source=contract&workerId=${workerId}`}>
 <Button
 type="button"
 variant="secondary"
 ...
 uploadContractId={uploadContractId}
 handleUploadSigned={handleUploadSigned}
...
interface SignedContractUploadModalProps {
 uploadContract: WorkerContractRecord | null;
 onClose: () => void;
 uploadMutation: any;
 uploadContractId: string;
 handleUploadSigned: () => void;
 selectedFile: File | null;
 setSelectedFile: (file: File | null) => void;
 signedAt: string;
 setSignedAt: (val: string) => void;
 getContractType: (contract: WorkerContractRecord) => string;
}

export function SignedContractUploadModal({
 uploadContract,
 onClose,
 uploadMutation,
 uploadContractId,
 ...
 <p className="text-xs font-semibold uppercase tracking-[0.12em] text-foreground-soft">
 Contrato
 </p>
 ...
 <label className="grid cursor-pointer gap-3 rounded-2xl border border-dashed border-slate-300 bg-muted px-5 py-6 text-center hover:border-indigo-400 hover:bg-indigo-50/40">
 <UploadCloud className="size-8 text-indigo-600" />
 <span className="text-sm font-semibold text-foreground">
 {selectedFile ? selectedFile.name : "Selecciona el contrato firmado"}
 </span>
 <span className="text-xs text-foreground-soft">PDF o imagen escaneada</span>
 <Input
 type="file"
 accept=".pdf,image/*"
 className="hidden"
