...
export function RequestsWorkspace({ section }: RequestsWorkspaceProps) {
  ...
  const myRequestsQuery = useRequests({
    scope: mainScope,
    filters: myFilters,
    enabled: isAuthenticated && section === "my-requests",
    includeStats: false,
  });
  const pendingRequestsQuery = useRequests({
    scope: "pending",
  ...
  const editingRequestQuery = useQuery({
  ...
  const stats = statsQuery.data ?? requestDefaultStats;
  const detailRequest = detailQuery.data ?? null;
  const editingRequest = mapDetailToFormData(editingRequestQuery.data);
  ...
  const content = (() => {
    if (section === "my-requests") {
      if (myRequestsQuery.listQuery.isLoading && !myRequestsQuery.listQuery.data) {
        return <LoadingPanel title="Cargando solicitudes." />;
      }

      if (myRequestsQuery.listQuery.isError && !myRequestsQuery.listQuery.data) {
        return (
          <ErrorState
            title="No se pudieron cargar las solicitudes"
            description={getErrorMessage(myRequestsQuery.listQuery.error)}
            onRetry={() => void myRequestsQuery.listQuery.refetch()}
          />
        );
      }

      const listData = myRequestsQuery.listQuery.data;
    ...
    if (section === "new-request") {
      if (requestTypesQuery.isLoading && !requestTypes.length) {
        return <LoadingPanel title="Cargando tipos de solicitud." />;
      }

      if (requestTypesQuery.isError && !requestTypes.length) {
        return (
          <ErrorState
            title="No se pudieron cargar los tipos de solicitud"
            description={getErrorMessage(requestTypesQuery.error)}
            onRetry={() => void requestTypesQuery.refetch()}
          />
        );
      }

      if (editingRequestId && editingRequestQuery.isLoading && !editingRequestQuery.data) {
        return <LoadingPanel title="Cargando la solicitud para edicion." />;
      }

      if (editingRequestId && editingRequestQuery.isError && !editingRequestQuery.data) {
        return (
          <ErrorState
            title="No se pudo cargar la solicitud"
            description={getErrorMessage(editingRequestQuery.error)}
            onRetry={() => void editingRequestQuery.refetch()}
          />
        );
      }
    ...
    if (section === "pending-requests") {
      if (pendingRequestsQuery.listQuery.isLoading && !pendingRequestsQuery.listQuery.data) {
        return <LoadingPanel title="Cargando solicitudes pendientes." />;
      }

      if (pendingRequestsQuery.listQuery.isError && !pendingRequestsQuery.listQuery.data) {
        return (
          <ErrorState
            title="No se pudieron cargar las solicitudes pendientes"
            description={getErrorMessage(pendingRequestsQuery.listQuery.error)}
            onRetry={() => void pendingRequestsQuery.listQuery.refetch()}
          />
        );
      }

      const listData = pendingRequestsQuery.listQuery.data;
  ...
  return (
    <>
      ...
      <RequestDetailModal
        ...
        errorDescription={detailQuery.isError ? getErrorMessage(detailQuery.error) : undefined}
      ...
      <ExportReportModal
        ...
        tableData={(section === "pending-requests" ? pendingRequestsQuery.listQuery.data?.items : myRequestsQuery.listQuery.data?.items)?.map(item => ({
          ...item,
