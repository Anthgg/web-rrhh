  ...
  const columns = [
    {
      ...
      render: (row: any) => (
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-xs text-slate-500">DNI: {row.doc}</span>
        </div>
      ),
    },
    {
      ...
      render: (row: any) => <span className="text-sm font-medium text-slate-700">{row.crew}</span>,
    },
    {
      key: "location",
      header: "Obra Actual",
    ...
    },
    {
      ...
      render: (row: any) => <span className="text-sm text-slate-600">{row.assignedAt}</span>,
    },
    {
      key: "status",
      header: "Estado",
        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden">
          {activeTab === "movements" ? (
            <div className="p-16 text-center">
              ...
              <h4 className="text-lg font-bold text-slate-800">Historial de Movimientos</h4>
              <p className="text-sm text-slate-500 mt-1">El historial detallado se implementarÃ¡ al conectar el endpoint del servidor.</p>
            </div>
          ) : (
            <DataTable
              columns={columns}
