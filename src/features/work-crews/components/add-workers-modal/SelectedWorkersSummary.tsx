...
export function SelectedWorkersSummary({ selectedWorkers, onRemove }: SelectedWorkersSummaryProps) {
 ...
 return (
 <div className="bg-muted border-t border-border p-4 shrink-0">
 ...
 <div key={w.id} className="flex items-center gap-1.5 px-2.5 py-1.5 bg-card border border-border rounded-lg shadow-sm group transition-colors hover:border-rose-200">
 <span className="text-xs font-medium text-foreground max-w-[120px] truncate">{w.fullName}</span>
 <button
 onClick={() => onRemove(w.id)}
 className="text-muted-foreground hover:text-rose-500 p-0.5 rounded-full hover:bg-rose-50"
 title="Quitar"
 >
 <X className="size-3" />
 </button>
