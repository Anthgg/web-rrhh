...
export function WorkLocationCard({ location, onViewDetail, onExport }: WorkLocationCardProps) {
 ...
 return (
 <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col transition-all hover:shadow-md hover:border-indigo-200 group">
 {/* Header */}
 <div className="p-5 border-b border-border flex items-start justify-between relative overflow-hidden">
 ...
 <h3 className="font-bold text-foreground text-base truncate pr-2">{location.name}</h3>
 ...
 <span className="inline-flex items-center gap-1 bg-muted text-muted-foreground px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border border-border">
 Inactiva
 </span>
 )}
 {hasCoordinates ? (
 ...
 <p className="text-sm text-muted-foreground truncate mt-1 flex items-center gap-1.5" title={location.address}>
 ...
 <div className="bg-card p-3 rounded-xl border border-border/60 shadow-sm flex flex-col justify-between">
 <span className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5 mb-1 uppercase tracking-wider">
 <div className="size-1.5 rounded-full bg-emerald-500" />
 Base
 </span>
 <span className="font-bold text-foreground text-lg">{baseCrew}</span>
 </div>
 <div className="bg-card p-3 rounded-xl border border-border/60 shadow-sm flex flex-col justify-between">
 <span className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5 mb-1 uppercase tracking-wider">
 ...
 <span className="font-bold text-foreground text-lg">{tempReceived}</span>
 </div>
 <div className="bg-card p-3 rounded-xl border border-border/60 shadow-sm flex flex-col justify-between">
 <span className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5 mb-1 uppercase tracking-wider">
 ...
 <span className="font-bold text-foreground text-lg">{tempSent}</span>
 </div>
 <div className="bg-card p-3 rounded-xl border border-border/60 shadow-sm flex flex-col justify-between">
 <span className="text-[11px] font-semibold text-muted-foreground flex items-center gap-1.5 mb-1 uppercase tracking-wider">
 ...
 <span className="font-bold text-foreground text-lg">{totalMovements}</span>
 </div>
 </div>
 </div>
 ...
 <Button onClick={onViewDetail} className="flex-1 bg-foreground hover:bg-secondary text-white rounded-xl shadow-sm">
 Ver detalle
 </Button>
 {onExport && (
 <Button variant="secondary" onClick={onExport} className="rounded-xl border-border text-muted-foreground hover:bg-muted">
 Exportar
 </Button>
 )}
 {hasCoordinates && (
 <Button 
 variant="secondary" 
 className="px-3 rounded-xl border-border text-blue-600 hover:bg-blue-50 hover:border-blue-200" 
 title="Ver en mapa"
