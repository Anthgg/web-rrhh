...
export function WorkTeamStats({ crews }: WorkTeamStatsProps) {
  ...
  const totalWorkers = crews.reduce((acc, curr) => acc + (curr.active_workers_count || 0), 0);
  
  // Count unique active locations
  const uniqueLocations = new Set(crews.filter(c => c.work_location_id).map(c => c.work_location_id)).size;

  // Sum of all temporarily moved workers across all crews
  const temporarilyMovedWorkers = crews.reduce((acc, curr) => acc + (curr.temporarily_moved_workers_count || 0), 0); 
  const monthlyMovements = crews.reduce((acc, curr) => acc + (curr.total_movements || 0), 0);
    ...
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
      {cards.map((card, idx) => {
        ...
        return (
          <div 
            key={idx} 
            className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col justify-between group hover:border-slate-300 transition-colors"
            title={card.tooltip}
          >
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider truncate mr-2">{card.label}</p>
