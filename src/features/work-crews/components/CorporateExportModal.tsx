    ...
    >
      <div className="grid lg:grid-cols-[1fr_300px] gap-8">
        <div className="flex flex-col gap-8">
          ...
          {/* Tipo de Reporte */}
          <section>
            <h4 className="text-sm font-bold text-slate-900 mb-3">1. Tipo de reporte</h4>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? "border-indigo-600 bg-indigo-50/50 ring-1 ring-indigo-600/20" : "border-slate-200 bg-white hover:border-slate-300"
                    )}
                  >
                    <div className={cn("p-2 rounded-lg mt-0.5", isSelected ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-500")}>
                      <Icon className="size-4" />
                    </div>
                    <div>
                      <p className={cn("text-sm font-semibold", isSelected ? "text-indigo-900" : "text-slate-800")}>{type.label}</p>
          ...
          <section>
            <h4 className="text-sm font-bold text-slate-900 mb-3">2. Modalidad de datos</h4>
            <div className="grid sm:grid-cols-2 gap-3">
              <button
                onClick={() => setExportMode("filters")}
                className={cn(
                  "p-4 rounded-xl border text-left transition-all",
                  exportMode === "filters" ? "border-slate-800 bg-slate-900 text-white shadow-md" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                )}
              >
                ...
                <p className={cn("text-xs leading-relaxed", exportMode === "filters" ? "text-slate-300" : "text-slate-500")}>
              ...
              <button
                ...
                className={cn(
                  "p-4 rounded-xl border text-left transition-all",
                  !isTableModeAvailable && "opacity-50 cursor-not-allowed bg-slate-50",
                  isTableModeAvailable && exportMode === "table" ? "border-slate-800 bg-slate-900 text-white shadow-md" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                )}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-sm flex items-center gap-2">
                ...
                <p className={cn("text-xs leading-relaxed", exportMode === "table" ? "text-slate-300" : "text-slate-500")}>
              ...
              {selectedFormat === "excel" ? (
                <div className="flex flex-wrap gap-2">
                  {tableColumns.map(col => {
                    ...
                    return (
                      <label key={col.key} className={cn("flex items-center gap-2 px-3 py-1.5 rounded-lg border cursor-pointer select-none transition-colors", checked ? "bg-white border-indigo-200 shadow-sm" : "bg-transparent border-transparent text-slate-500 hover:bg-slate-200/50")}>
                  ...
                  {tableColumns.length === 0 && <p className="text-xs text-slate-500 italic p-2">No hay columnas configuradas para seleccionar.</p>}
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-2">
                  {PDF_SECTIONS.map(sec => {
                    ...
                    return (
                      <label key={sec.id} className={cn("flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer select-none transition-colors", checked ? "bg-white border-rose-200 shadow-sm" : "bg-transparent border-transparent text-slate-500 hover:bg-slate-200/50")}>
            ...
            {visibleFilters.length > 0 ? (
              <div className="flex flex-col gap-2">
                {visibleFilters.map(([key, val]) => (
                  <div key={key} className="flex flex-col bg-white border border-slate-100 p-2 rounded-lg">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{activeFilterLabels[key] || defaultFilterLabel(key)}</span>
                    <span className="text-xs font-medium text-slate-800 truncate" title={String(val)}>{String(val)}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center p-4 bg-white rounded-xl border border-slate-100">
                <p className="text-xs text-slate-500 font-medium">No hay filtros activos.</p>
                <p className="text-[11px] text-slate-400 mt-1">Se exportarÃ¡ toda la informaciÃ³n disponible en la base de datos.</p>
              </div>
            )}
          </div>
