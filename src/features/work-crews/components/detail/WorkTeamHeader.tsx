...
export function WorkTeamHeader({ crew, onExportPdf, onExportExcel }: WorkTeamHeaderProps) {
 ...
 return (
 <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-card border-b border-border px-6 py-5 shrink-0 shadow-sm gap-4">
 <div className="flex items-center gap-4">
 <Button variant="secondary" className="h-10 gap-2 rounded-xl border-border text-muted-foreground hover:text-foreground hover:bg-muted" onClick={() => router.back()}>
 ...
 <h2 className="text-xl font-bold text-foreground">{crew?.name || "Detalle de Cuadrilla"}</h2>
 <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${isActive ? "bg-emerald-100 text-emerald-700" : "bg-muted text-muted-foreground"}`}>
 ...
 <p className="text-sm text-muted-foreground mt-0.5">Administra el supervisor, obra y trabajadores asignados</p>
 )}
 </div>
 </div>
 </div>
 ...
 <Button variant="secondary" className="h-10 gap-2 border-border text-muted-foreground hover:bg-muted">
