...
export function MainWorkLocationCard({ crew, onChangeLocation }: MainWorkLocationCardProps) {
 ...
 <div>
 <h3 className="text-sm font-bold text-foreground">Obra Principal</h3>
 <p className="text-xs text-muted-foreground">Ubicación base de la cuadrilla</p>
 </div>
 </div>
 <Button variant="ghost" className="h-8 px-3 text-xs font-medium bg-muted text-muted-foreground hover:bg-muted rounded-full" onClick={onChangeLocation}>
 Cambiar Obra
 </Button>
 </div>
 ...
 <p className="text-sm text-muted-foreground mb-4">Esta cuadrilla no tiene una obra principal configurada.</p>
 <Button variant="secondary" onClick={onChangeLocation}>Configurar ubicación</Button>
 </div>
 ) : (
 <div className="space-y-4">
 <div className="flex items-start gap-3">
 <Compass className="size-4 text-muted-foreground mt-0.5" />
 <div>
