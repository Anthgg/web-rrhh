...
export function DepartmentFormModal({ isOpen, onClose, departmentData }: DepartmentFormModalProps) {
 ...
 return <DepartmentFormModalContent onClose={onClose} departmentData={departmentData} />;
}

function DepartmentFormModalContent({ onClose, departmentData }: Omit<DepartmentFormModalProps, "isOpen">) {
 ...
 />
 <label htmlFor="is_active_dept" className="text-sm font-medium text-foreground">
 Departamento activo
 </label>
 </div>
 )}
 ...
 <button form="department-form" type="submit" disabled={!isDirty || mutation.isPending} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-primary-hover disabled:pointer-events-none disabled:opacity-50">
