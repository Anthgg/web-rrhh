...
export function RolesWorkspace({ hideHeader }: { hideHeader?: boolean }) {
 ...
 <span className="font-medium text-foreground">{role.name || role.label}</span>
 {isSystemRole && (
 ...
 header: "Descripción",
 render: (role: RoleDefinition) => (
 <span className="text-muted-foreground max-w-[200px] truncate block" title={role.description}>
 {role.description || "-"}
 </span>
 ),
 },
