...
export function RequestReportsPanel({
  ...
  const [prevQuickFields, setPrevQuickFields] = useState({
  ...
  const availableColumns = useMemo(
    () => (columnsQuery.data?.length ? columnsQuery.data : requestReportColumnsCatalog),
    [columnsQuery.data],
  );
  ...
  const previewRows = useMemo(
    ...
    [effectiveSelectedColumns, previewQuery.data?.items],
  );
  ...
  const [prevUrlTemplateId, setPrevUrlTemplateId] = useState(urlTemplateId);
  const [prevTemplatesData, setPrevTemplatesData] = useState(templatesQuery.data);

  if (urlTemplateId !== prevUrlTemplateId || templatesQuery.data !== prevTemplatesData) {
    setPrevUrlTemplateId(urlTemplateId);
    setPrevTemplatesData(templatesQuery.data);

    if (urlTemplateId && templatesQuery.data?.some((entry) => entry.id === urlTemplateId)) {
      const template = templatesQuery.data.find((entry) => entry.id === urlTemplateId);
      if (template) {
  ...
  return (
    <div className="grid gap-6">
      ...
      <ReportDownloadActions
        totalRecords={previewQuery.data?.total ?? 0}
          ...
          {previewQuery.isLoading && !previewQuery.data ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 border border-red-100 rounded-3xl bg-red-50/50 gap-4">
              <div className="grid gap-1 text-center">
                ...
                <p className="text-xs text-red-700/80 max-w-md">{getErrorMessage(previewQuery.error)}</p>
              </div>
              <button
                type="button"
