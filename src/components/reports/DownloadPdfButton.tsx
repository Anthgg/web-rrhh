    ...
    return (
      <div className="relative group inline-block">
        <Button
          ...
          disabled
        >
          <FileText className={cn("size-4 text-slate-400", !showIconOnly && "mr-2")} />
          {!showIconOnly && (children || "Exportar PDF")}
        </Button>
        <div className="absolute bottom-full left-1/2 z-50 mb-2 w-48 -translate-x-1/2 scale-95 rounded-lg bg-slate-900 px-3 py-2 text-center text-xs font-semibold text-white opacity-0 shadow-xl transition-all duration-200 group-hover:scale-100 group-hover:opacity-100 pointer-events-none">
          No tienes permisos para descargar este reporte corporativo.
          <div className="absolute top-full left-1/2 -mt-1 size-2 -translate-x-1/2 rotate-45 bg-slate-900" />
        </div>
      </div>
    );
  }
