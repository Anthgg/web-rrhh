...
export function ReportsGeneratorPage() {
  ...
  const previewQuery = useReportPreview(previewRequest);
  ...
  const [prevTemplateFromQuery, setPrevTemplateFromQuery] = useState(selectedTemplateFromQuery);
  const [prevTemplatesData, setPrevTemplatesData] = useState(templatesQuery.data);

  if (
    selectedTemplateFromQuery !== prevTemplateFromQuery ||
    templatesQuery.data !== prevTemplatesData
  ) {
    setPrevTemplateFromQuery(selectedTemplateFromQuery);
    setPrevTemplatesData(templatesQuery.data);

    if (selectedTemplateFromQuery && templatesQuery.data?.length) {
      const template = templatesQuery.data.find((entry) => entry.id === selectedTemplateFromQuery);
      if (template) {
  ...
  const filtersSummary = useMemo(() => {
    ...
    if (filters.requestType) {
      const type = requestTypesQuery.data?.find((entry) => entry.id === filters.requestType);
    ...
    return summary;
  }, [filters, isManager, requestTypesQuery.data, workers]);
