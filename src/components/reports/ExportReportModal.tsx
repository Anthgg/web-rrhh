...
export function ExportReportModal({
 ...
 exportMode === "filters" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
 )}>
 <Filter className="size-4" />
 </div>
 <span className="font-semibold text-foreground">Usar filtros actuales</span>
 </div>
 <p className="text-xs text-foreground-soft leading-5">
 ...
 !isTableModeAvailable && "opacity-50 cursor-not-allowed bg-muted border-border",
 isTableModeAvailable && exportMode === "table"
 ? "border-primary bg-primary/5 ring-1 ring-primary hover:scale-[1.01] hover:shadow-md"
 : isTableModeAvailable && "border-border bg-card hover:scale-[1.01] hover:shadow-md"
 )}
 >
 <div className="flex items-center gap-2">
 <div className={cn(
 "flex size-8 items-center justify-center rounded-xl",
 exportMode === "table" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"
 )}>
 <Table className="size-4" />
 </div>
 <span className="font-semibold text-foreground">Usar datos de la tabla</span>
 </div>
 <p className="text-xs text-foreground-soft leading-5">
 ...
 <div key={key} className="flex items-center gap-1.5 rounded-lg border border-border bg-card px-2.5 py-1 text-xs text-muted-foreground">
 ...
 <p className="text-xs text-foreground-soft italic">Ningún filtro activo (se exportará todo el listado oficial).</p>
 )}
 ...
 <span id="columns-selector-label" className="text-sm font-semibold text-foreground">Seleccionar columnas a exportar</span>
 ...
 key={col.key}
 className={cn(
 "flex items-center gap-3 rounded-xl border p-3 hover:bg-muted transition cursor-pointer select-none",
 isChecked ? "border-slate-300 bg-muted/70" : "border-border bg-card"
 )}
 >
 <input
 type="checkbox"
 className="size-4.5 rounded border-slate-300 text-primary focus:ring-primary accent-brand"
 checked={isChecked}
 ...
 <div className="mt-2 rounded-xl bg-muted border border-border p-3 text-xs text-foreground-soft flex items-center gap-2">
