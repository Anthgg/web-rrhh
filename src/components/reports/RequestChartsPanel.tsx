...
const ResponsiveContainer = dynamic(() => import("recharts").then(mod => mod.ResponsiveContainer), { ssr: false, loading: () => <div className="flex h-full items-center justify-center"><Loader2 className="size-6 animate-spin text-primary" /></div> });
...
export function RequestChartsPanel({
 ...
 <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/10 bg-card/5 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-slate-200">
 ...
 className="border-white/10 bg-card/5 text-white"
 >
 ...
 <div className="h-[320px] animate-pulse rounded-[1.75rem] bg-card/5" />
 </div>
 ) : !chartData.length ? (
 <EmptyState
 ...
 <div className="grid gap-4 rounded-[1.75rem] border border-white/10 bg-card/[0.03] p-4">
 ...
 <div
 key={key}
 className="rounded-[1.25rem] border border-white/10 bg-card/[0.04] px-4 py-3"
 >
