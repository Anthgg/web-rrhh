...
export function CreatePositionModal({ isOpen, onClose }: CreatePositionModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      reset({ departmentId: "", areaId: "", name: "", description: "" });
      setGlobalError(null);
    }
  }, [isOpen, reset]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
      <div className="flex w-full max-w-lg flex-col rounded-2xl bg-white shadow-xl overflow-hidden">
        <div className="flex items-center justify-between border-b border-slate-100 p-6 shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Nuevo Puesto</h2>
            <p className="mt-1 text-sm text-slate-500">Crea un nuevo puesto o cargo dentro de un área.</p>
          </div>
        ...
        <div className="flex shrink-0 items-center justify-end gap-3 border-t border-slate-100 p-6 bg-slate-50/50">
          ...
          <button form="position-form" type="submit" disabled={!isDirty || mutation.isPending} className="flex items-center justify-center gap-2 rounded-xl bg-brand px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-brand-hover disabled:pointer-events-none disabled:opacity-50">
