...
export function CreateWorkLocationModal({ isOpen, onClose }: CreateWorkLocationModalProps) {
 ...
 return <CreateWorkLocationModalContent onClose={onClose} />;
}

function CreateWorkLocationModalContent({ onClose }: Pick<CreateWorkLocationModalProps, "onClose">) {
 ...
 <div>
 <h2 className="text-xl font-bold text-foreground">Nuevo Lugar de Trabajo</h2>
 <p className="mt-1 text-sm text-muted-foreground">Registra una nueva sede o ubicación para los trabajadores.</p>
 </div>
 ...
 <div className="mt-4">
 <p className="mb-2 block text-sm font-medium text-foreground">Seleccionar ubicación en el Mapa</p>
 <LocationPickerMap
 ...
 <input
 type="checkbox"
 aria-label="Lugar de trabajo activo"
 checked={field.value}
 ...
 />
 <span className="text-sm text-foreground">Lugar de trabajo activo</span>
 </div>
 ...
 <button form="location-form" type="submit" disabled={!isDirty || mutation.isPending} className="flex items-center justify-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-primary-hover disabled:pointer-events-none disabled:opacity-50">
