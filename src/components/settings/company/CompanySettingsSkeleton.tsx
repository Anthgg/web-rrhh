...
export function CompanySettingsSkeleton() {
 ...
 <section className="-mx-4 border-b border-border bg-muted p-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
 ...
 <Card className="rounded-lg border-border bg-card p-2 shadow-sm">
 <div className="hidden gap-1 md:flex">
 {Array.from({ length: 5 }).map((_, index) => (
 <div key={index} className="h-10 flex-1 animate-pulse rounded-md bg-muted" />
 ))}
 </div>
 <div className="h-11 animate-pulse rounded-lg bg-muted md:hidden" />
 </Card>
 ...
 <div className="h-5 w-[34rem] max-w-full animate-pulse rounded-full bg-muted" />
 </div>
 <div className="h-28 w-full animate-pulse rounded-lg bg-muted sm:w-44" />
 </div>

 <div className="grid gap-4 md:grid-cols-2">
 {Array.from({ length: 6 }).map((_, index) => (
 <div key={index} className="h-36 animate-pulse rounded-lg border border-border bg-muted" />
 ))}
 </div>
 </Card>

 <Card className="hidden rounded-lg border-border bg-card p-4 shadow-sm xl:grid">
