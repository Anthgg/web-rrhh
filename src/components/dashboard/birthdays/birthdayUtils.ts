...
export function enhanceBirthdayWorker(
 ...
 const birthday = worker.birthday || "2000-01-01";
 const { monthIndex, day } = getBirthdayParts(birthday);
 const nextBirthday = getNextBirthdayDate(birthday, today);
 const daysUntilBirthday = getDaysUntilBirthday(birthday, today);
...
export function compareBirthdayMonthDay(a: BirthdayWorker, b: BirthdayWorker) {
 const first = getBirthdayParts(a.birthday || "2000-01-01");
 const second = getBirthdayParts(b.birthday || "2000-01-01");
...
export function filterBirthdaysByDay(workers: BirthdayWorker[], date: Date) {
 ...
 return getBirthdayMonthDayKey(worker.birthday) === key;
 });
}
...
export function filterBirthdaysByMonth(workers: BirthdayWorker[], monthIndex: number) {
 return workers.filter((worker) => {
 if (!worker.birthday) return false;
 return getBirthdayParts(worker.birthday).monthIndex === monthIndex;
 });
}
