...
export function PlaceSearchInput({ id, ariaLabel = "Buscar lugar", disabled, onSelect }: PlaceSearchInputProps) {
 ...
 const cleanupSearch = useEffectEvent(() => {
 abortRef.current?.abort();
 ...
 className="h-10 w-full rounded-xl border border-border bg-card py-2 pl-9 pr-9 text-sm text-foreground outline-none transition placeholder:text-muted-foreground focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 disabled:cursor-not-allowed disabled:opacity-50"
 autoComplete="off"
 spellCheck={false}
 />
 {/* Right icon */}
 ...
 className="absolute left-0 top-full z-[500] mt-1 w-full overflow-hidden rounded-xl border border-border bg-card shadow-xl"
 >
 {status === "loading" ? (
 /* Loading state */
 ...
 <li key={place.place_id != null ? String(place.place_id) : primaryLabel(place)}>
 <button
 type="button"
 role="option"
 aria-selected={suggestions[activeIndex] === place}
 onMouseDown={(e) => {
 e.preventDefault(); // prevent input blur
 handleSelect(place);
 }}
 className={`flex w-full items-start gap-2.5 rounded-lg px-3 py-2.5 text-left transition-colors ${
 suggestions[activeIndex] === place
 ...
 suggestions[activeIndex] === place ? "text-indigo-500" : "text-indigo-300"
 }`}
 />
