...
import { getApiErrorDetails, getApiErrorMessage } from "@/lib/api/error-handlers";
...
export function QuickOrganizationBuilderModal({ isOpen, onClose }: QuickOrganizationBuilderModalProps) {
 ...
 await Promise.all(values.areas.map(async (areaData) => {
 ...
 await Promise.all(areaData.positions.map((positionData) => {
 setProgressStep(`Creando puesto: ${positionData.name}...`);
 return organizationService.createPosition({
 areaId: newArea.id,
 ...
 logger.error("Error in QuickBuilder:", error);
 setGlobalError({
 message: getApiErrorMessage(error, "Ocurrio un error al crear la estructura. Es posible que el departamento se haya creado parcialmente."),
 details: getApiErrorDetails(error) ?? error,
 });
 setIsSaving(false);
 setProgressStep("");
 }
 ...
 <div className="flex w-full max-w-4xl max-h-[90vh] flex-col rounded-2xl bg-card shadow-xl overflow-hidden">
 <div className="flex items-center justify-between border-b border-border p-6 shrink-0 bg-muted/50">
 <div>
 ...
 <button
 type="button"
 className="rounded-lg p-2 text-muted-foreground hover:bg-slate-200 hover:text-muted-foreground transition"
 onClick={onClose}
 disabled={isSaving}
 >
 <X className="size-5" />
 ...
 <div className="overflow-y-auto p-6 flex-1 bg-muted/30">
 ...
 <h3 className="text-base font-semibold text-foreground">Estructura Interna (Áreas y Puestos)</h3>
 <Button
 type="button"
 variant="secondary"
 onClick={() => appendArea({ name: "", positions: [{ name: "" }] })}
 disabled={isSaving}
 className="gap-2 text-primary border-primary/20 hover:bg-primary/5 h-9 px-3 text-sm"
 >
 ...
 <div key={areaField.id} className="rounded-xl border border-border bg-card p-5 shadow-sm relative overflow-hidden">
 ...
 <input {...field} disabled={isSaving} className="form-input bg-muted" placeholder="Ej: Logística, Mantenimiento..." />
 </FieldFrame>
 )}
 />

 {/* POSITIONS FOR THIS AREA */}
 <div className="pl-4 border-l-2 border-border mt-4 space-y-3">
 ...
 type="button"
 variant="ghost"
 className="text-rose-700 hover:text-rose-800 hover:bg-rose-50 -mt-1 -mr-2"
 ...
 <div className="flex shrink-0 items-center justify-between border-t border-border p-6 bg-muted">
 <div className="text-sm font-medium text-muted-foreground">
 {isSaving && (
 <div className="flex items-center gap-2 animate-pulse text-primary">
 <div className="size-4 animate-spin rounded-full border-2 border-primary/30 border-t-brand" />
 {progressStep}
 </div>
 )}
 ...
 type="submit"
 disabled={isSaving}
 className="gap-2 bg-foreground text-white hover:bg-secondary"
 >
...
function PositionsList({ control, areaIndex, isSaving }: { control: any, areaIndex: number, isSaving: boolean }) {
 ...
 type="button"
 variant="ghost"
 className="text-muted-foreground hover:text-rose-600 px-2 h-8"
 ...
 <button
 type="button"
 className="text-xs font-semibold text-primary hover:text-primary-hover flex items-center gap-1 mt-1 transition-colors"
 onClick={() => append({ name: "" })}
 disabled={isSaving}
 >
