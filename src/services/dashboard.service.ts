...
import { normalizeBirthdayWorker, normalizeUser } from "@/lib/api/normalizers";
...
export const dashboardService = {
  ...
  async getAdminAttendanceDashboard(): Promise<AdminAttendanceDashboard> {
    const [
      ...
      apiClient<UserMeResponse>("/api/users/me"),
    ...
    const userPayload = asRecord(userRes)?.data;
    const normalizedUser = normalizeUser(userPayload ?? userRes);
    const rawRole = asRecord(userPayload)?.role;
    ...
    const dashboardUser: DashboardUser = {
      ...
      role: normalizedUser.role !== "unknown" ? normalizedUser.role : asString(rawRole),
    };
