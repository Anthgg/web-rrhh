...
export function OnboardingStatusChecklist({ workerId }: OnboardingStatusChecklistProps) {
 ...
 <span className="text-xs text-muted-foreground font-medium">Cargando estado del onboarding...</span>
 </div>
 );
 }

 if (isStatusError || !statusResponse) {
 return (
 ...
 <div className="flex justify-between items-center bg-muted border border-border rounded-xl p-4">
 <div>
 <h4 className="text-xs font-semibold text-foreground uppercase tracking-wider">Progreso del Registro</h4>
 ...
 onClick={() => void refetchStatus()}
 className="h-8 px-2.5 rounded-lg border border-border bg-card hover:bg-muted text-muted-foreground flex items-center gap-1.5 text-xs"
 >
 {isStatusRefetching ? (
 ...
 item.isCompleted ? "text-emerald-900" : "text-foreground"
 }`}
 >
 {item.label}
 </span>
