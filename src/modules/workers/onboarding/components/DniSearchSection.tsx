...
export function DniSearchSection({ setValue, dniValue, onChangeDni }: DniSearchSectionProps) {
 ...
 <h3 className="mb-2 text-sm font-bold text-foreground uppercase tracking-wider">Consulta por DNI</h3>
 ...
 className="w-full h-10 rounded-xl bg-indigo-600 font-medium text-white hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 {isSearching ? (
 <>
 ...
 {fullName && (
 <div className="ml-6 text-xs text-emerald-900 bg-emerald-50/40 p-2.5 rounded-xl border border-emerald-100/50 max-w-md animate-fadeIn">
 <span className="font-semibold text-emerald-800">Nombre completo: </span>
 <span className="font-bold text-emerald-950">{fullName}</span>
 </div>
 )}
 </div>
 )}
